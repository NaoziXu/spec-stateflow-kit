...
一个让 claw agent 与 Claude Code 基于 spec 文档协同工作的套件——将复杂任务拆解为可追踪、状态驱动、可随时恢复的结构化开发流水线。

> ⚠️ 这是一个 **fake skill**（占位导航入口），不包含可执行逻辑。本文件仅作为套件根目录的 skill 标识。要查看完整的架构设计、组件说明、状态机原理与部署拓扑，请阅读同目录下的 **README.md**。
