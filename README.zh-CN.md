...
│   ├── SKILL.md                     ← 安装 / 卸载逻辑
│   ├── claude-sample.md             ← CLAUDE.md 模板（决策规则）
│   ├── claude-reference-sample.md   ← CLAUDE-reference.md 模板（详细参考）
│   ├── spec-env.json.example        ← 环境配置模板
│   └── test-prompts.json            ← 10 个测试场景
│
├── spec-stateflow/                  ← 核心工作流引擎（运行在 Claude Code 内）
│   ├── SKILL.md                     ← 四阶段工作流 + 状态机
│   └── test-prompts.json
│
├── spec-task-progress/              ← 进度查询（运行在 Agent 内）
│   ├── SKILL.md
...
安装器 (`spec-stateflow-kit-installer`) 管理完整的生命周期 — 复制技能、注入 CLAUDE.md 内容、写入配置 — 用户无需手动触碰任何路径。
...
**角色：** 生命周期管理器
**功能：** 以 2 种模式安装或卸载整个套件：
- **安装**（7步）：检查 Claude Code → 配置环境 → 复制 4 个 agent 技能 → 校验路径 → 注入 CLAUDE.md + CLAUDE-reference.md（自动语言检测）→ 安装 spec-stateflow 到 Claude Code
- **卸载**（8步）：确认 → 停止监控进程 → 移除所有组件包括状态文件

**关键设计：** 自动检测 CLAUDE.md 语言（CJK 比例 > 0.3 → 中文翻译），3 策略移除确保干净卸载，卸载时完整清理（安装器本身也会被移除）。
...
**关键设计：** `.project` 文件机制实现项目目录持久化，工作区保护（有未提交变更 → 在提示词中附加警告），`--dangerously-skip-permissions` 实现非交互执行。
...
**关键设计：** 与 spec-driver 解耦。进度检查器是独立的 `claude -p` 进程（spec-task-progress skill）。守护进程状态存于 `/tmp/`，进度状态存于 `{SPEC_PATH}/progress.json`，检查器 PID 存于 `{SPEC_PATH}/checker.json`。
